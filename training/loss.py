...
def D_logistic_r2(G, D, opt, training_set, minibatch_size, reals, labels, gamma=10.0):
    ...
    return loss, tf.cast( reg, tf.float16 )
...
def G_logistic_ns_pathreg(G, D, opt, training_set, minibatch_size, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2.0):
    ...
    with tf.name_scope('PathReg'):
        ...
        # Compute |J*y|.
        pl_noise = tf.cast( tf.random.normal(tf.shape(fake_images_out)) / np.sqrt(np.prod(G.output_shape[2:])), tf.float16 )
        ...
        pl_lengths = tf.cast( tf.sqrt(tf.reduce_mean(tf.reduce_sum(tf.square(pl_grads), axis=2), axis=1)), tf.float16 )
        pl_lengths = autosummary('Loss/pl_lengths', pl_lengths)

        # Track exponential moving average of |J*y|.
        with tf.control_dependencies(None):
            pl_mean_var = tf.Variable(name='pl_mean', trainable=False, initial_value=0.0, dtype=tf.float16)
