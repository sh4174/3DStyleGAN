...
def create_3d_image_grid_y(images, grid_size=None):
    assert images.ndim == 4 or images.ndim == 5
    # NCDHW
    num, img_w, img_h, img_d = images.shape[0], images.shape[-1], images.shape[-2], images.shape[-3]
    ...
    print( "")
    if grid_size is not None:
        grid_w, grid_d = tuple(grid_size)
    else:
        grid_w = max(int(np.ceil(np.sqrt(num))), 1)
        grid_d = max((num - 1) // grid_w + 1, 1)

    grid = np.zeros(list(images.shape[1:-2]) + [grid_d * img_d, grid_w * img_w], dtype=images.dtype)

    for idx in range(num):
        x = (idx % grid_w) * img_w
        y = (idx // grid_w) * img_d
        if images.ndim == 4:
            grid[..., y : y + img_d, x : x + img_w] = images[idx][ :, int(img_h//2), : ]
        else:
            grid[..., y : y + img_d, x : x + img_w] = images[idx][ 0, :, int(img_h//2), : ]

    return grid


def create_3d_image_grid_z(images, grid_size=None):
    assert images.ndim == 4 or images.ndim == 5
    # NCDHW
    num, img_w, img_h, img_d = images.shape[0], images.shape[-1], images.shape[-2], images.shape[-3]
...
def save_3d_image_grid_z(images, filename, drange=[0,1], grid_size=None):
    pil_image = create_3d_image_grid_z(images, grid_size)
    convert_3d_to_pil_image( pil_image, drange).save(filename)

def save_3d_image_grid_y(images, filename, drange=[0,1], grid_size=None):
    pil_image = create_3d_image_grid_y(images, grid_size)
    convert_3d_to_pil_image( pil_image, drange).save(filename)


def apply_mirror_augment_3d(minibatch):
    mask = np.random.rand(minibatch.shape[0]) < 0.5
    minibatch = np.array(minibatch)
    minibatch[mask] = minibatch[mask, :, ::-1, :, :]
    return minibatch
